...
class TestedTransport : public PluginTa2TwoSixStubTransport {
public:
    ...
    TestedTransport(ITransportSdk *sdk,
                    std::unordered_map<LinkID, std::shared_ptr<MockLink>> &mockLinks) :
        PluginTa2TwoSixStubTransport(sdk), transportSdk(sdk), mockLinks(mockLinks) {}
...
class TestPluginTa2TwoSixStubTransport : public ::testing::Test {
public:
    ::testing::NiceMock<MockTransportSdk> sdk;
    ChannelProperties channelProps;
    std::unordered_map<LinkID, std::shared_ptr<MockLink>> mockLinks;

    TestPluginTa2TwoSixStubTransport() {
    ...
    std::unique_ptr<PluginTa2TwoSixStubTransport> createTransport() {
        return std::make_unique<TestedTransport>(&sdk, mockLinks);
    }
};

TEST_F(TestPluginTa2TwoSixStubTransport, should_refuse_to_create_link_when_max_links_exceeded) {
...
TEST_F(TestPluginTa2TwoSixStubTransport, should_refuse_to_create_link_when_invalid_role_link_side) {
...
TEST_F(TestPluginTa2TwoSixStubTransport, should_create_link) {
    channelProps.currentRole.linkSide = LS_CREATOR;
    auto transport = createTransport();
...
TEST_F(TestPluginTa2TwoSixStubTransport, should_load_link_address) {
    channelProps.currentRole.linkSide = LS_LOADER;
    auto transport = createTransport();
...
TEST_F(TestPluginTa2TwoSixStubTransport, should_not_load_link_addresses) {
    auto transport = createTransport();
...
TEST_F(TestPluginTa2TwoSixStubTransport, should_create_link_from_address) {
    channelProps.currentRole.linkSide = LS_CREATOR;
    auto transport = createTransport();
...
TEST_F(TestPluginTa2TwoSixStubTransport, should_destroy_link) {
    auto transport = createTransport();
...
TEST_F(TestPluginTa2TwoSixStubTransport, should_create_encoding_params_for_fetch_action) {
    auto transport = createTransport();
...
TEST_F(TestPluginTa2TwoSixStubTransport, should_create_encoding_params_for_post_action) {
    auto transport = createTransport();
...
TEST_F(TestPluginTa2TwoSixStubTransport, should_enqueue_content_for_fetch_action) {
    auto transport = createTransport();
    Action action{8675309, 42, "{\"linkId\":\"LinkID_1\",\"type\":\"fetch\"}"};
    ASSERT_EQ(COMPONENT_OK, transport->enqueueContent({}, action, {}));
}

TEST_F(TestPluginTa2TwoSixStubTransport, should_enqueue_content_for_post_action) {
    auto transport = createTransport();
    ...
    Action action{8675309, 42, "{\"linkId\":\"LinkID_1\",\"type\":\"post\"}"};
    ASSERT_EQ(COMPONENT_OK, transport->enqueueContent({}, action, {0x31, 0x41, 0x59}));
}

TEST_F(TestPluginTa2TwoSixStubTransport, should_dequeue_content_for_fetch_action) {
    auto transport = createTransport();
    Action action{8675309, 42, "{\"linkId\":\"LinkID_1\",\"type\":\"fetch\"}"};
    ASSERT_EQ(COMPONENT_OK, transport->dequeueContent(action));
}

TEST_F(TestPluginTa2TwoSixStubTransport, should_dequeue_content_for_post_action) {
    auto transport = createTransport();
    ASSERT_EQ(COMPONENT_OK, transport->createLink(1u, "LinkID_1"));
...
TEST_F(TestPluginTa2TwoSixStubTransport, should_perform_fetch_action) {
    auto transport = createTransport();
    ASSERT_EQ(COMPONENT_OK, transport->createLink(2u, "LinkID_2"));
    EXPECT_CALL(*mockLinks["LinkID_2"], fetch(std::vector<RaceHandle>{7}))
        .WillOnce(::testing::Return(COMPONENT_OK));
    Action action{8675309, 42, "{\"linkId\":\"LinkID_2\",\"type\":\"fetch\"}"};
    ASSERT_EQ(COMPONENT_OK, transport->doAction({7}, action));
}

TEST_F(TestPluginTa2TwoSixStubTransport, should_perform_post_action) {
    auto transport = createTransport();
    ASSERT_EQ(COMPONENT_OK, transport->createLink(2u, "LinkID_2"));
