...
TEST_F(TestLink, fetch_should_return_error_when_shutdown) {
    link.shutdown();
    ASSERT_EQ(COMPONENT_ERROR, link.fetch({1}));
}

TEST_F(TestLink, fetch_should_return_error_when_action_queue_full) {
    for (int i = 0; i < 10; ++i) {
        ASSERT_EQ(COMPONENT_OK, link.fetch({static_cast<RaceHandle>(i)}));
    }
    ASSERT_EQ(COMPONENT_ERROR, link.fetch({10}));
}
...
TEST_F(TestLinkActionThread, action_thread_should_execute_queued_fetch_action) {
    ...
    ASSERT_EQ(COMPONENT_OK, link.fetch({7}));
    waitForActionToBeExecuted();

    actionExecuted = false;

    ASSERT_EQ(COMPONENT_OK, link.fetch({8}));
    waitForActionToBeExecuted();
}
