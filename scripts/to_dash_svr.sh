...
if [ ${6:-0} -eq 1 ]; then
    /usr/local/lib/race/ta2/PluginTA2SRIDash/scripts/video_wedge -mcudensity 100 -bpf 2 -nfreqs 12 -maxfreqs 12 -cover "${1}" -message "${3}" -jpeg_quality 30 -dir /ramfs/sriDash -wedge_dir $wedge_dir $_vw_opts
else
    /usr/local/lib/race/ta2/PluginTA2SRIDash/scripts/video_wedge -mcudensity 100 -bpf 1 -nfreqs 12 -maxfreqs 12 -cover "${1}" -message "${3}" -jpeg_quality 30 -dir /ramfs/sriDash -wedge_dir $wedge_dir $_vw_opts
fi

_mp4_cover_name=`basename "{1}" .mp4`
_cover_frame_dir="/ramfs/sriDash/.cover/${_mp4_cover_name}"
