...
_wedge=$(which wedge.p3)
_unwedge=$(which unwedge.p3)

if [ -z "$_wedge" ]; then
    _wedge=$_dirname/wedge.p3
    _unwedge=$_dirname/unwedge.p3
fi
...
case "$_cmd" in
	...
	jpegoptim "/tmp/$$_steg.jpg" > /dev/null 2>&1
        _status=$?

        if [ $_status -eq 0 ]; then
    	    "$_unwedge" ${@:1:$#-1} /tmp/$$_steg.jpg /tmp/$$_out.msg > /dev/null 2>&1
