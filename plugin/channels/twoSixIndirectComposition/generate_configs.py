...
from race_python_utils import ta1_request_utils
from race_python_utils import ta1_utils
from race_python_utils import ta2_link_utils
...
CHANNEL_ID = "twoSixIndirectComposition"
try:
    with open(
...
def main() -> None:
    ...
    if cli_args.ta1_request_file:
        ta1_request = file_utils.read_json(cli_args.ta1_request_file)
    else:
        ta1_request = ta1_request_utils.generate_ta1_request_from_range_config(
            range_config,
            [CHANNEL_PROPERTIES],
            "multicast",
            "indirect",
        )
    ta1_request_utils.validate_ta1_request(ta1_request, range_config)

    # Prepare the dir to store configs in, check for previous values and overwrite
    file_utils.prepare_ta2_config_dir(cli_args.config_dir, cli_args.overwrite)
...
def generate_configs(
    ...
    """
    Purpose:
        Generate the plugin configs based on range config and ta1 request
    Args:
        range_config: range config to generate against
        ta1_request: requested links to generate against
    ...
    file_utils.write_json(
        fulfilled_ta1_request, f"{config_dir}/fulfilled-ta1-request.json"
    )
...
def generate_genesis_link_addresses(
    ...
    """
    Purpose:
        Generate indirect TA2 links utilizing the two six whiteboard
    Args:
        range_config: range config to generate against
        ta1_request: requested links to generate against
        local_override: Ignore range config services. Use Two Six Local Information.
    ...
    Returns:
        ...
        fulfilled_ta1_request: which ta1 links in the request were fufilled
    """

    link_addresses = {}
    fulfilled_ta1_request: Dict[str, Any] = {"links": []}
    ...
    # Build link profile
    link_profile = additional_attributes
    link_profile.update(
        {
            "multicast": True,
    ...
    base_client_hashtag = "cpp_clients_{link_idx}"
    base_server_hashtag = "cpp_servers_{link_idx}"
        ...
        link_addresses[sender_node].append(
            {
                ...
                "address": f'{{"port" : {two_six_whiteboard_details.get("""port""", None)}, "hostname" : "{two_six_whiteboard_details.get("""hostname""", None)}", "hashtag": "{hashtag}"}}',
    ...
    if len(fulfilled_ta1_request["links"]) < len(ta1_request["links"]):
        logging.warning(
            f"Not all links fufilled: {len(fulfilled_ta1_request['links'])} of "
            f"{len(ta1_request['links'])} fulfilled"
        )
    else:
...
def build_link_properties() -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    send_properties = ta2_link_utils.generate_link_properties_dict(
        "send",
    ...
    receive_properties = ta2_link_utils.generate_link_properties_dict(
        "receive",
        link_transmission_type,
        link_reliability,
        link_latency_ms,
...
def get_cli_arguments() -> argparse.Namespace:
    ...
    optional.add_argument(
        ...
        help=(
            "Requested links from TA1. Configs should generate only these"
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        stream=sys.stdout,
        level=LOG_LEVEL,
        format="[generate_ta2_cpp_indirect_configs] %(asctime)s.%(msecs)03d %(levelname)s %(message)s",
