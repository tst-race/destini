...
#define TRACE_SRI_METHOD(...) TRACE_METHOD_BASE(PluginTA2SRI_MJPGEncodingCpp, ##__VA_ARGS__)
#define TRACE_SRI_FUNCTION(...) TRACE_FUNCTION_BASE(PluginTA2SRI_MJPGEncodingCpp, ##__VA_ARGS__)

const char *PluginTA2SRI_MJPGEncoding::name = "SRIMJPGEncoding";


PluginTA2SRI_MJPGEncoding::PluginTA2SRI_MJPGEncoding(IEncodingSdk *_sdk, const PluginConfig &_pluginConfig) : sdk(_sdk), pluginConfig(_pluginConfig) {
    if (this->sdk == nullptr) {
        throw std::runtime_error ("PluginTA2SRI_MJPGEncoding: sdk parameter is NULL");
    }
    ...
    auto codecJSONPath = CLICodec::DirFilename ("mjpg_codec.json");   // works out to be /etc/race/etc/<channel>

    // Hack:  For now, the strategy is to keep all codec descriptors in the same SRI Decomposed Plugin directory.
    // auto codecJSONPath = CLICodec::DirFilename("/etc/race/plugins/unix/x86_64/PluginTA2SRIDecomposed/mjpg_codec.json");
    ...
    if (fileExists (codecJSON)) {
         ...
         if (cliCodec == nullptr || !cliCodec->isGood ())
             throw std::runtime_error ("PluginTA2SRI_MJPGEncoding: bad or incomplete JSON (" + codecJSONPath + ")");
    }
    else
        throw std::runtime_error ("PluginTA2SRI_MJPGEncoding: JSON (" + codecJSONPath + ") not found");

    sdk->updateState(COMPONENT_STATE_STARTED);
}

ComponentStatus PluginTA2SRI_MJPGEncoding::onUserInputReceived(RaceHandle handle, bool answered,
...
EncodingProperties PluginTA2SRI_MJPGEncoding::getEncodingProperties() {
    TRACE_SRI_METHOD();

    return {cliCodec->encodingTime (), cliCodec->mimeType ()};
}

SpecificEncodingProperties PluginTA2SRI_MJPGEncoding::getEncodingPropertiesForParameters(
...
ComponentStatus PluginTA2SRI_MJPGEncoding::encodeBytes(RaceHandle handle,
                                                     const EncodingParameters &params,
    ...
    logInfo ("PluginTA2SRI_MJPGEncoding::encodeBytes, nMsgIn: " + std::to_string (nMsgIn));
...
ComponentStatus PluginTA2SRI_MJPGEncoding::decodeBytes(RaceHandle handle,
                                                     const EncodingParameters &params,
                                                     const std::vector<uint8_t> &bytes) {
    TRACE_SRI_METHOD(handle, params.linkId, params.type, params.encodePackage, params.json);
