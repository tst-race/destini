...
const char *PluginTa2TwoSixBase64Encoding::name = "base64";

PluginTa2TwoSixBase64Encoding::PluginTa2TwoSixBase64Encoding(IEncodingSdk *_sdk) : sdk(_sdk) {
    if (this->sdk == nullptr) {
        throw std::runtime_error("PluginTa2TwoSixBase64Encoding: sdk parameter is NULL");
    }
    sdk->updateState(COMPONENT_STATE_STARTED);
}

ComponentStatus PluginTa2TwoSixBase64Encoding::onUserInputReceived(RaceHandle handle, bool answered,
...
EncodingProperties PluginTa2TwoSixBase64Encoding::getEncodingProperties() {
    TRACE_METHOD();

    return {0, "text/plain"};
}

SpecificEncodingProperties PluginTa2TwoSixBase64Encoding::getEncodingPropertiesForParameters(
...
ComponentStatus PluginTa2TwoSixBase64Encoding::encodeBytes(RaceHandle handle,
                                                           const EncodingParameters &params,
...
ComponentStatus PluginTa2TwoSixBase64Encoding::decodeBytes(RaceHandle handle,
                                                           const EncodingParameters &params,
                                                           const std::vector<uint8_t> &bytes) {
    TRACE_METHOD(handle, params.linkId, params.type, params.encodePackage, params.json);
