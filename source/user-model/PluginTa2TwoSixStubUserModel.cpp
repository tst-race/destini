#include "PluginTa2TwoSixStubUserModel.h"
...
PluginTa2TwoSixStubUserModel::PluginTa2TwoSixStubUserModel(IUserModelSdk *sdk) : sdk(sdk) {
    // No user input requests are needed, so user model is ready right away
    sdk->updateState(COMPONENT_STATE_STARTED);
}

ComponentStatus PluginTa2TwoSixStubUserModel::onUserInputReceived(RaceHandle handle, bool answered,
...
UserModelProperties PluginTa2TwoSixStubUserModel::getUserModelProperties() {
    TRACE_METHOD();
    // TODO implement this
    return {};
}

std::shared_ptr<LinkUserModel> PluginTa2TwoSixStubUserModel::createLinkUserModel(
    const LinkID &linkId) {
    return std::make_shared<LinkUserModel>(linkId, nextActionId);
}

ComponentStatus PluginTa2TwoSixStubUserModel::addLink(const LinkID &link,
...
ComponentStatus PluginTa2TwoSixStubUserModel::removeLink(const LinkID &link) {
    TRACE_METHOD(link);
    {
        std::lock_guard<std::mutex> lock(mutex);
...
ActionTimeline PluginTa2TwoSixStubUserModel::getTimeline(Timestamp start, Timestamp end) {
...
ComponentStatus PluginTa2TwoSixStubUserModel::onTransportEvent(const Event & /* event */) {
    TRACE_METHOD();
...
IUserModelComponent *createUserModel(const std::string &usermodel, IUserModelSdk *sdk,
    ...
    return new PluginTa2TwoSixStubUserModel(sdk);
}
