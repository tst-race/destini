                ...
                void        *pMsgOut, size_t *nMsgOut)
{
  ...
  *_nMsgOut = 0;
  ...
  if      (_pid < 0)   /* fork () failed */
    ...
    _SS_DIAGPRINT ("::run (" << codecPath << "): base forked child status:" << _status << ", _nMsgOut = " << *_nMsgOut);
...
#define _CODEC_DECOMP_VAL         "Decomposed-TA2"
...
CLICodec::SetDirname (const std::string &dirName)
{
  ...
  // logDebug(" CLICodec:SetDirname (\"" << dirName << "\")" );
  ...
  _lastMediaPtr (nullptr)
{
  do {        // "Do once" loop to expedite graceful error exit
    ...
    auto _isDecomposedTA2 = _getJSONString (jRoot, _CODEC_FORMAT_KEY) == _CODEC_DECOMP_VAL;
