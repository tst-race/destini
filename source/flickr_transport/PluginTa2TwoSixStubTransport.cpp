#include "PluginTa2TwoSixStubTransport.h"
...
PluginTa2TwoSixStubTransport::PluginTa2TwoSixStubTransport(ITransportSdk *sdk) :
    sdk(sdk),
...
ComponentStatus PluginTa2TwoSixStubTransport::onUserInputReceived(RaceHandle handle, bool answered,
...
TransportProperties PluginTa2TwoSixStubTransport::getTransportProperties() {
    TRACE_METHOD();
    return {
        // supportedActions
        {
...
LinkProperties PluginTa2TwoSixStubTransport::getLinkProperties(const LinkID &linkId) {
    TRACE_METHOD(linkId);
    return links.get(linkId)->getProperties();
}

bool PluginTa2TwoSixStubTransport::preLinkCreate(const std::string &logPrefix, RaceHandle handle,
                                                 const LinkID &linkId,
                                                 LinkSide invalideRoleLinkSide) {
    logDebug(logPrefix + "in PluginTa2TwoSixStubTransport::preLinkCreate");
...
ComponentStatus PluginTa2TwoSixStubTransport::postLinkCreate(const std::string &logPrefix,
                                                             RaceHandle handle,
                                                             const LinkID &linkId,
...
std::shared_ptr<Link> PluginTa2TwoSixStubTransport::createLinkInstance(
    ...
    link->start();
    logDebug("in PluginTaTwoSixStubTransport::createLinkInstance after link->start()");
    return link;
}

ComponentStatus PluginTa2TwoSixStubTransport::createLink(RaceHandle handle, const LinkID &linkId) {
    TRACE_METHOD(handle, linkId);
    logDebug(logPrefix + "in PluginTa2TwoSixStubTransport::createLink");
    ...
    logDebug(logPrefix + "PluginTa2TwoSixStubTransport::createLink hashtag:" + address.hashtag);
...
ComponentStatus PluginTa2TwoSixStubTransport::loadLinkAddress(RaceHandle handle,
                                                              const LinkID &linkId,
                                                              const std::string &linkAddress) {
    TRACE_METHOD(handle, linkId, linkAddress);
    logDebug(logPrefix + "PluginTa2TwoSixStubTransport::loadLinkAddress link address:" + linkAddress);
...
ComponentStatus PluginTa2TwoSixStubTransport::loadLinkAddresses(
...
ComponentStatus PluginTa2TwoSixStubTransport::createLinkFromAddress(
...
ComponentStatus PluginTa2TwoSixStubTransport::destroyLink(RaceHandle handle, const LinkID &linkId) {
    TRACE_METHOD(handle, linkId);
...
std::vector<EncodingParameters> PluginTa2TwoSixStubTransport::getActionParams(
    const Action &action) {
...
ComponentStatus PluginTa2TwoSixStubTransport::enqueueContent(
...
ComponentStatus PluginTa2TwoSixStubTransport::dequeueContent(const Action &action) {
...
ComponentStatus PluginTa2TwoSixStubTransport::doAction(const std::vector<RaceHandle> &handles,
                                                       const Action &action) {
    TRACE_METHOD(handles, action.actionId);
    logInfo("FlickrStubTransport(PluginTa2TwoSixStub): In doAction");

    try {
        ActionJson actionParams = nlohmann::json::parse(action.json);
	std::shared_ptr<Link> l = links.get(actionParams.linkId);
	logInfo("FlickrStubTransport: HELLO");
	
        switch (actionParams.type) {
            case ACTION_FETCH:
	      logInfo("FlickrStubTransport(PluginTa2TwoSixStub): In doAction Fetch");
	      return l->fetch(std::move(handles));
	      //return links.get(actionParams.linkId)->fetch(std::move(handles));

            case ACTION_POST:
	      logInfo("FlickrStubTransport(PluginTa2TwoSixStub): In doAction Post");
...
ITransportComponent *createTransport(const std::string &transport, ITransportSdk *sdk,
    ...
    return new PluginTa2TwoSixStubTransport(sdk);
}
