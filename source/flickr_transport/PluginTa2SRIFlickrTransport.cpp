#include "PluginTa2SRIFlickrTransport.h"
...
PluginTa2SRIFlickrTransport::PluginTa2SRIFlickrTransport(ITransportSdk *sdk) :
    sdk(sdk),
...
ComponentStatus PluginTa2SRIFlickrTransport::onUserInputReceived(RaceHandle handle, bool answered,
...
TransportProperties PluginTa2SRIFlickrTransport::getTransportProperties() {
    TRACE_METHOD();
    return {
        // supportedActions
        {
...
LinkProperties PluginTa2SRIFlickrTransport::getLinkProperties(const LinkID &linkId) {
    TRACE_METHOD(linkId);
    return links.get(linkId)->getProperties();
}

bool PluginTa2SRIFlickrTransport::preLinkCreate(const std::string &logPrefix, RaceHandle handle,
                                                 const LinkID &linkId,
                                                 LinkSide invalideRoleLinkSide) {
    logDebug(logPrefix + "in PluginTa2SRIFlickrTransport::preLinkCreate");
...
ComponentStatus PluginTa2SRIFlickrTransport::postLinkCreate(const std::string &logPrefix,
                                                             RaceHandle handle,
                                                             const LinkID &linkId,
...
std::shared_ptr<Link> PluginTa2SRIFlickrTransport::createLinkInstance(
  ...
  std::cout << "========================In createLinkInstance, about to get new tags" << "\n";
    ...
    link->start();
    logDebug("in PluginTa2SRIFlickrTransport::createLinkInstance after link->start()");
    return link;
}


ComponentStatus PluginTa2SRIFlickrTransport::createLink(RaceHandle handle, const LinkID &linkId) {
    TRACE_METHOD(handle, linkId);
    logDebug(logPrefix + "in PluginTa2SRIFlickrTransport::createLink");
    ...
    logDebug(logPrefix + "PluginTa2SRIFlickrTransport::createLink hashtag:" + address.hashtag);
...
ComponentStatus PluginTa2SRIFlickrTransport::loadLinkAddress(RaceHandle handle,
                                                              const LinkID &linkId,
                                                              const std::string &linkAddress) {
    TRACE_METHOD(handle, linkId, linkAddress);
    logDebug(logPrefix + "PluginTa2SRIFlickrTransport::loadLinkAddress link address:" + linkAddress);
...
ComponentStatus PluginTa2SRIFlickrTransport::loadLinkAddresses(
...
ComponentStatus PluginTa2SRIFlickrTransport::createLinkFromAddress(
    ...
    logDebug("PluginTa2SRIFlickrTransport::createLinkFromAddress calling createLinkInstance:");
    LinkAddress address = nlohmann::json::parse(linkAddress);
    LinkProperties properties = defaultLinkProperties;
    auto link = createLinkInstance(linkId, address, properties);
...
ComponentStatus PluginTa2SRIFlickrTransport::destroyLink(RaceHandle handle, const LinkID &linkId) {
    TRACE_METHOD(handle, linkId);
...
std::vector<EncodingParameters> PluginTa2SRIFlickrTransport::getActionParams(
    const Action &action) {
...
ComponentStatus PluginTa2SRIFlickrTransport::enqueueContent(
...
ComponentStatus PluginTa2SRIFlickrTransport::dequeueContent(const Action &action) {
...
ComponentStatus PluginTa2SRIFlickrTransport::doAction(const std::vector<RaceHandle> &handles,
                                                       const Action &action) {
    TRACE_METHOD(handles, action.actionId);
    logDebug("FlickrStubTransport(PluginTa2TwoSixStub): In doAction");

    try {
        ActionJson actionParams = nlohmann::json::parse(action.json);
	std::shared_ptr<Link> l = links.get(actionParams.linkId);
	// logDebug("FlickrStubTransport: HELLO");
	
        switch (actionParams.type) {
            case ACTION_FETCH:
	      logDebug("FlickrStubTransport(PluginTa2TwoSixStub): In doAction Fetch");
	      return l->fetch(std::move(handles));
	      //return links.get(actionParams.linkId)->fetch(std::move(handles));

            case ACTION_POST:
	      logDebug("FlickrStubTransport(PluginTa2TwoSixStub): In doAction Post");
...
ITransportComponent *createTransport(const std::string &transport, ITransportSdk *sdk,
    ...
    return new PluginTa2SRIFlickrTransport(sdk);
}
