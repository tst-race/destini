...
CONTNAME="plugin_builder_decomposed_cpp_ta2"


# Docker Image Values
RACE_CONTAINER_REGISTRY="gitlab.race.twosixlabs.com:4567"
RACE_GITLAB_GROUP="race-common"
RACE_COMPILE_IMAGE_REPO="racesdk"
...
COMMAND="./build_artifacts-hacked.sh"
...
Arguments:
    ...
    -g [value], --group [value], --group=[value]
        Specify a gitlab group for the container registry. Defaults to '${RACE_GITLAB_GROUP}'.
    --race-version [value], --race-version=[value]
        Specify the RACE version. Defaults to '${RACE_VERSION}'.
    -N, --name
        Specify the container name.  Defaults to '${CONTNAME}'
    -h, --help
        Print this message.
    --
...
Examples: 
    ...
    ./build_artifacts_in_docker_image.sh --race-version=1.4.0
...
if [ "${LOCAL}" = true ]; then
    ...
    RACE_COMPILE_IMAGE="ghcr.io/tst-race/private-race-core/race-sdk:main"
    #RACE_COMPILE_IMAGE="${RACE_CONTAINER_REGISTRY}/${RACE_GITLAB_GROUP}/${RACE_COMPILE_IMAGE_REPO}/${RACE_COMPILE_IMAGE_NAME}:${RACE_VERSION}"
else
    RACE_COMPILE_IMAGE="${RACE_CONTAINER_REGISTRY}/${RACE_GITLAB_GROUP}/${RACE_COMPILE_IMAGE_REPO}/${RACE_COMPILE_IMAGE_NAME}:${RACE_VERSION}"
...
docker run $_dashRM \
    ...
    ${DOCKER_ARGS} \
    "${RACE_COMPILE_IMAGE}" \
    "${COMMAND}" ${BUILD_ARGS} "$@" > docker_command.txt
